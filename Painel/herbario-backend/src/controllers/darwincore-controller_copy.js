...
export const obterModeloDarwinCore = (request, response, next) => {
  Promise.resolve()
    ...
    .then((retorno) => {
      ...
      // console.log(retorno[0].usuarios);
      // console.log(retorno[0].coletores);
      console.log(retorno[0].tombos_coletores);
      ...
      retorno.forEach((tombo) => {
        ...
        const paranaNome =
          tombo.locais_coletum && tombo.locais_coletum.cidade
            ? tombo.locais_coletum.cidade.estado.nome
            : '';
        const cidadeNome =
          tombo.locais_coletum && tombo.locais_coletum.cidade
            ? tombo.locais_coletum.cidade.nome
            : '';
        const vegetacao =
          tombo.locais_coletum && tombo.locais_coletum.vegetaco
            ? tombo.locais_coletum.vegetaco.nome
            : '';
        ...
        if (tombo.coletores && tombo.coletores.length > 0) {
          coletores += tombo.coletores.map((coletor) => {
            console.log(coletor.nome);
            return nomeParaIniciais(coletor.nome);
          });

          nomeIdentificador += tombo.coletores.map((coletor) => coletor.nome);

          console.log('nomeIdentificador', nomeIdentificador);
          // console.log(
          //   '🚀 ~ file: darwincore-controller.js:290 ~ retorno.forEach ~ coletores:',
          //   coletores
          // );
        }
        ...
        if (tombo.tombos_fotos && tombo.tombos_fotos.length > 0) {
          tombo.tombos_fotos.forEach((foto) => {
            const dataAtualizacao = moment(tombo.updated_at).format(
              'YYYY-MM-DD'
            );
            ...
            let linha = [
              ...
              `${license}\tUTFPR\t{"barcode":${
                foto.codigo_barra
              }}\tBr:UTFPR:HCF:${tombo.hcf.toString()}`,
              ...
              `${
                tombo.longitude
              }\t${'WGS84'}\t${'GPS'}\t${'Plantae'}\t${familiaNome}\t${generoNome}\t`,

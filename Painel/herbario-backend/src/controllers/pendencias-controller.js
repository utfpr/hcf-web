...
export const aprovarComJson = (alteracao, hcf, response, next) => {
    const parametros = {};
    ...
    return Promise.resolve()
        .then(() => {
            ...
            if(alteracao.coletores){
                for(var i = 0; i < alteracao.coletores.length; i ++ ){
                ...
                .then(() => {
                    if (objetoAlterado.sub_familia_id) {
                        Subfamilia.findOne({
                            ...
                            console.log("esta e minha subfamilia\n\n\n\n\n", subfamilia);
                            if (subfamilia) {
                                parametros.subfamilia = subfamilia;
                            }
                        })
...
export function aceitarPendencia(request, response, next) {
    const pendencia_id = request.params.pendencia_id; 
    return Promise.resolve()
        .then(() => Alteracao.findOne({
            where: {
                ativo: true,
...
export function avaliaPendencia(request, response, next) {
    const {pendencia_id } = request.params;
